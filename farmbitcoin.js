...
const bot = new Telegraf("831295697:AAFyoD1x5nQrdwDu8OLFVRG8mkIaniVZz-Y");
const Scene = require('telegraf/scenes/base')
const session = require('telegraf/session')
const Stage = require('telegraf/stage')
const { enter,leave } = Stage
var rest = require('restler');
var btc='1f7d2e80-984a-500f-b433-4f249925e6a0'
    ...
    con.query("SELECT id FROM account WHERE id=" + id, function (err, result, fields) {
        ...
        if (message.text == start&&result.length===0) {
            ...
            con.query("insert into `account` SET ?", user, function (error, results) {
                ...
                console.log(error)
                ctx.reply('welcome ' + ctx.from.first_name + ' to Ethshop.\n\n🔹Own shops\n🔹Collect income\n🔹Exchange income for 💰\n🔹Earn real Money in ETH', Markup
                    .keyboard([
...
bot.action('👤Refferal link',ctx=>{
    ctx.editMessageText('https://t.me/ethshopbot?start='+ctx.from.id,Extra
        .HTML()
        .markup((m) => m.inlineKeyboard([
                    ...
                    address.getTransactions({}, function (err, txs) {
                        if (txs.length === 0) {
                            ...
                            con.query(sqli, function (err, response) {
                                ...
                                ctx.telegram.sendMessage('@bitcoinshoppays', 'new deposit of ' + transactions + ' ETH by ' + ctx.from.first_name + '\n\nhttps://etherscan.io/address/' + res[0].depoaddre)
                            })
                        }
                    })
                })
        ...
        if (results[0].depoaddre== null) {
            client.getAccount(btc, function (err, account) {
                account.createAddress(null, function (err, address) {
                    ...
                    ctx.replyWithHTML('<b>⚡Top up balance</b>\n\nYou can buy game currency for purchasing shops and (💰) using BCH. To top up your balance, simply send<b> any amount</b> of ETH to this address\n\n<code>' + adress + '</code>\n\nThe ETH will be automatically credited to the balance at the rate of\n' + '<b>0.1 ETH = 100,000 💰</b>\n' + '<b>0.01 ETH = 10,000 💰</b>\n' + '<b>0.001 ETH = 1,000 💰</b>\n' + '<b>0.0001 ETH = 100 💰 </b>etc.')
                    var ide =ctx.from.id
            ...
            con.query(sqla, function (error, results, fields) {
                ctx.replyWithHTML('<b>⚡Top up balance</b>\n\nYou can buy game currency for purchasing shops and (💰) using ETH. To top up your balance, simply send<b> any amount</b> of ETH to this address\n\n<code>' + results[0].depoaddre + '</code>\n\nThe ETH will be automatically credited to the balance at the rate of\n' + '<b>0.1 ETH = 100,000 💰</b>\n' + '<b>0.01 ETH = 10,000 💰</b>\n' + '<b>0.001 ETH = 1,000 💰</b>\n' + '<b>0.0001 ETH = 100 💰 etc.</b>')
                    .then(() => {
            ...
            con.query(sql, function (error, results, fields) {
                ...
                client.getAccount(btc, function (err, account) {
                    account.sendMoney({
                        ...
                        ctx.telegram.sendMessage('@bitcoinshoppays', 'New withdrawal of ' + payout + ' ETH by ' + ctx.from.first_name + '\n\nhttps://etherscan.io/address/' + results[0].withdrawadd)
                        var user = ctx.from.id
                        ...
                        con.query(sqla)
                        ctx.replyWithHTML('Your withdrawal of ' + payout + ' ETH is being processed', Markup
                            .keyboard([
                                ['🏦Shops', '💵Income'], // Row1 with 2 buttons
                                ['💸Balance', '👨‍👧‍👦Refferals'], // Row2 with 2 buttons
                                ['⚖️Exchange', '📈Stastistics'],
